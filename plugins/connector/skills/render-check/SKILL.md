...
| 認証情報の管理 | credentials-manager プラグイン（オプション）または各コネクタスキルの対話取得フォールバック（本スキル自体は認証情報を使用しない） |
...
| 入力 | モード | 動作 |
|------|-------|------|
| ユーザー直接の依頼（単体起動） | 対話 | ターゲット記法が不明な場合は `AskUserQuestion` で確認し、判定結果と修正案を報告する |
| 他スキル（backlog / azure 等）からのゲート呼び出し | 非対話 | 指定されたターゲットで検証し、判定結果を呼び出し元に返す（質問しない） |
