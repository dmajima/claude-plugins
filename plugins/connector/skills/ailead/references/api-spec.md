...
| `callSummary.topics` | Array | トピック別の要約 |
| `callSummary.topics[].category` | String | カテゴリ: `SHARE` / `DISCUSSION` / `DECISION` / `CONCERN` / `SUGGESTION` / `DIALOGUE` / `SCHEDULE` |
...
- URL にはアクセストークンが含まれており、共有リンク有効期限内は認証なしでアクセス可能
- セグメント長は約 20〜36 秒
...
| `buildId` の更新 | デプロイごとに変更される。HTML から毎回動的に取得するため通常は問題ない |
