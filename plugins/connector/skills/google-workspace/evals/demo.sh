#!/usr/bin/env bash
# google-workspace スキルの構造検証を自動デモする再現可能スクリプト (Bash 版)
#
# 実装方針:
# - 本スキルの実動作は MCP ツール (mcp__claude_ai_Google_Drive__*) に依存する
#   ため、demo.sh では外部 API を一切呼ばない「構造検証」のみを行う
# - 検証内容: SKILL.md の存在 / frontmatter name=google-workspace / スキル内
#   references の存在 / プラグイン共通 references の存在 / evals ケースの存在
# - すべて読み取り専用チェック (ファイル変更・外部通信をしない)
# - MCP ツールを伴う実動作の確認は evals/case-01 〜 06 の手順による
...
SKILL_DIR=$(dirname "$SCRIPT_DIR")                 # skills/google-workspace
...
# --- デモシナリオ ---
write_section "google-workspace スキル構造検証デモ開始"
printf '  WhatIf モード: %s\n' "$whatif"
printf '  スキルディレクトリ:   %s\n' "$SKILL_DIR"
printf '  プラグインディレクトリ: %s\n' "$PLUGIN_DIR"
printf '\n'
printf '  実施するシナリオ:\n'
printf '    1. SKILL.md の存在と frontmatter (name: google-workspace) の検証\n'
...
# Step 1: SKILL.md の存在 + frontmatter name=google-workspace
write_section "Step 1: SKILL.md の存在と frontmatter 検証"
check_file "SKILL.md の存在" "$SKILL_DIR/SKILL.md"
if [[ "$whatif" == "true" ]]; then
  printf '  [WhatIf] frontmatter 検証: head -n 5 SKILL.md から "name: google-workspace" 行を検出\n'
else
  if head -n 5 "$SKILL_DIR/SKILL.md" 2>/dev/null | grep -q '^name: google-workspace$'; then
    printf '  [OK]   frontmatter name: google-workspace\n'
  else
    printf '  [FAIL] frontmatter に "name: google-workspace" が見つからない\n'
    fail_count=$((fail_count + 1))
  fi
fi
...
check_file "case-01 (ファイル検索)" "$SCRIPT_DIR/case-01_file_search.md"
check_file "case-02 (ドキュメント読取)" "$SCRIPT_DIR/case-02_read_document.md"
check_file "case-03 (ファイル作成)" "$SCRIPT_DIR/case-03_create_file.md"
...
printf '\n'
printf '    "Google Drive で議事録ファイルを検索して"          # case-01 (読み取り)\n'
printf '    "Google Drive にこの内容でファイルを作成して"      # case-03 (書き込み + 承認 UI)\n'
printf '\n'
printf '  書き込み系は AskUserQuestion 承認が必ず挟まる\n'
printf '  (期待動作の詳細は evals/case-03_create_file.md を参照)。\n'
...
printf '    - 実 MCP 動作 (case-01 / 03 等) を検証用環境で目視確認したか\n'
printf '\n'
