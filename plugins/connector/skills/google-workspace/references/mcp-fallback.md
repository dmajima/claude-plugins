...
AskUserQuestion({
  ...
  options: [
    ...
    {
      label: "直接対応する",
      description: "Google Drive API を直接使用します。OAuth2 Bearer Token が必要です。操作は限定的になります。"
    }
  ]
})
```
...
1. `credentials-manager` スキルで Google API トークンを取得する
   - entry key: `google-drive` / type: `api_key`
