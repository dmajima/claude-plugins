...
#### A-0-2. CLI 存在チェック（必要サブコマンドの照合）

`claude plugin --help` を実行し以下をすべて満たすことを確認する:

- exit code が 0
- 出力に **連続文字列** `marketplace update` を含む（Phase B で必要）
- 出力に **連続文字列** `update` がサブコマンド一覧形式（行頭またはインデント直後）として含まれる
  （Phase C/D/E で必要）
...
連続文字列照合（旧版の `marketplace` AND `update` の単純 AND）よりは精度が高いが、攻撃者が
シムを精巧に作れば回避可能。`which claude` / `Get-Command claude` で実行バイナリの絶対パスを
確認することをユーザに推奨（README「動作要件」参照）。
...
3. **第三手順（必須）— ブロック終端検出**: 抽出結果を Claude が走査し、`enabledPlugins` の値である
   `{` の対応する `}` を検出した時点で **それ以降のテキストを破棄** する（`mcpServers` 等が `-A` 範囲内に
   含まれた場合の情報漏洩防止）。具体的には:
...
> **直前** に XR-3（[cross-cutting-rules.md](../references/cross-cutting-rules.md)）のサニタイズを
> 必ず適用する。規則本体・列単位の例外・「文脈外」判定の具体ルール・テストケース・適用順序は
> すべて XR-3 SSOT を参照のこと。本コマンドは XR-3 を再定義しない。
