...
1. **第一手順（必須）**: `Grep` で `enabledPlugins` の開始ブロックを抽出（暫定範囲 500 行）:
   ...
   **Unicode エスケープによる回避防止（必須補助 Grep・早期警告ベストエフォート）**: 攻撃者が
   ...
   > **責務範囲**: 第一手順の補助 Grep は **早期警告のベストエフォート**。仮に補助 Grep を
   > すり抜けた難読化キー（例: 想定外の Unicode 形式 `\\u{xxxx}` 等）があっても、第二手順の
   > 正規表現照合（リテラル `"enabledPlugins"\s*:\s*\{` の厳密マッチ）が **構造的フェイルクローズ
   > の本体** として機能し、機密キーがメインコンテキストに混入するシナリオは構造的に排除される
   > （第二手順を通過しないため Phase A-Sec 全体が「対象なし（不正なスキーマ）」で終わる）。
...
3. **第三手順（必須）— ブロック終端検出（構造解析）**: 抽出結果を Claude が走査し、`enabledPlugins`
   ...
   - **`[` `]` は別カウンタとして並行追跡** し、`[` でレベル +1、`]` でレベル -1。`enabledPlugins`
     ...
     **異常検知の判定条件**: `{` `}` ネストレベルが 0 に戻った時点で **`[` カウンタが 1 以上のまま**
     であれば、`{...}` ブロック終端が `[...]` の外側で起きた構造異常 = 即エラー終了
...
> **状態保持要件（G-1 用）**: G-1 質問文で使用する `<bn>`（サーキットブレーカー作動 MP 件数）は
> **Phase B-1 / C / D / E 完了時点で確定し Phase G-1 直前まで保持** する（output-formats.md
> 「Phase G-1 質問文 → 変数定義」を参照）。
