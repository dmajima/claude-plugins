...
1. **第一手順（必須）**: `Grep` で `enabledPlugins` の開始ブロックを抽出（暫定範囲 500 行）:
   ...
   **Unicode エスケープによる回避防止（必須補助 Grep）**: 攻撃者が `"enabledPlugins"`
   （`e` を `e` で表現）等の Unicode エスケープでキーを難読化するケースに備え、以下の
   追加 Grep を実行する:

   ```text
   Grep(pattern: '\\\\u00[46]5nabledPlugins\|\\\\u0070\|\\\\u006[CcEe]', path: '<settings.json>', output_mode: 'count')
   ```

   検出件数が 1 件以上の場合、当該スコープを **「不正なスキーマ（Unicode エスケープによるキー難読化）」**
   としてエラー終了する（DoS サイレント発生防止のため早期失敗。エラー文言は output-formats.md
   「エラーメッセージ集約」を参照）。
