...
`apiKeyHelper` / `env` / `permissions` / `awsAuthRefresh` / `customApiKeyResponses` 等の機密キー。
**例示であり網羅ではない。検出規範は第四手順のホワイトリスト方式が SSOT**）はメインコンテキストに
一切載せない。以下の手順を **必ず順守**する（dry-run モードでも全手順を省略しない）:
...
3. **第三手順（必須）— ブロック終端検出（構造解析）**: 抽出結果を Claude が走査し、`enabledPlugins`
   ...
   - **`[` `]` は別カウンタとして並行追跡** し、`[` でレベル +1、`]` でレベル -1。`enabledPlugins`
     値はオブジェクト（`{...}`）であるため `[` `]` はネスト内のサブ値（プラグイン値が将来配列化
     された場合のみ）にしか出現しないが、**並行追跡することで第二手順の型ガードをすり抜けた
     `{` 偽装を二重に検知**する（`[` カウンタが 0 でないまま `}` で 0 に戻る等の異常を即エラー）
...
4. **第四手順（必須）— 事後検証ガード（フェイルクローズ二重化・パターン検証）**: 第三手順の
   ...
   **将来拡張への注意**: 現在の `enabledPlugins` スキーマはフラット 1 段ネスト
   ...
   サブキー（`enabled` / `config`）がインデント 2〜4 段で出現し、ホワイトリスト正規表現の
   インデント上限と整合しなくなる可能性がある。スキーマ拡張時は本第四手順の正規表現と
   インデント段数制限を再評価する。
