...
> ことでリリース対応関係を追跡可能にする。
>
> **`—` の意味**: 「最終 CLI 仕様確認日」列の `—` は **N/A（CLI 仕様に直接依存しない ADR）** を
> 示す。ADR-PU-001（プラグイン配布単位）/ ADR-PU-004（SSOT 配置）/ ADR-PU-006（サーキットブレーカー
...
- `commands/update-all.md`: 約 44 行（**v3.7.0 計測スナップショット ± 5 行誤差を許容**: フロントマター
...
- **コマンド本文の単純化**: 約 460 行（v2.x 当時）→ 約 44 行（v3.7.0 計測実測値・実装ロジックは
