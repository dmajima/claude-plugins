...
| コマンドから委譲される `target` | `all`（既定） |
| 既存状態 | マーケットプレイス 3 件・各 user/project/local スコープにプラグイン複数（複数 projectPath あり） |
...
- 未登録 MP の早期除外
- `installed_plugins.json` の `scope` / `projectPath` で全 projectPath のエントリを収集（target=all）
...
- `target=all` の場合、Project / Local プラグインは projectPath ごとにグルーピングして表示する（`output-formats.md` F-3（dry-run）参照）
...
|-----|-------|
| 変更系 CLI 呼び出し | なし |
| 標準出力（要約） | 「## 実行予定サマリ（dry-run）」（`output-formats.md` F-1（dry-run）フォーマット） |
| Project / Local プラグイン表示 | projectPath ごとのグルーピングテーブル（`output-formats.md` F-3（dry-run）フォーマット） |
| 終了状態 | 成功（exit 0） |
...
- `case-05_target_all.md`（同じ target=all だが mode = normal）
