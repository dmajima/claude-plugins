...
- **対象外（明示）**: **Phase A の `claude plugin marketplace list` 出力**は本サニタイズの対象外
  ...
  件数（整数）のみのため対象外。
  > **既知のリスク許容**: ユーザが `https://<TOKEN>@github.com/...` 形式の認証付き URL で
  > マーケットプレイスを登録している場合、`marketplace list` 出力に URL 埋め込みトークンが含まれる
  > 可能性がある。本プラグインはこの出力をメインコンテキストには表示しない（Phase A の内部処理のみで
  > 利用し、Phase F の結果報告には MP 名のみ抽出して使用）が、A-0-2 INFO や Phase F-2 で MP 名を
  > 表示する際に URL 部分を含めない設計が前提。実装時は `marketplace list` の URL 列を抽出処理から
  > 除外し MP 名のみを保持する。リスク評価は ADR-PU-005 Trade-offs を参照。
...
| パターン | 置換後 | 備考 |
|---------|-------|------|
| `(?i)(token\|password\|secret\|authorization\|bearer\|x-api-key)[:=\s]+[^\s,;)\]\}"'<>]{1,256}` | `<key>=***REDACTED***` | 汎用 key=value（空白区切りの `Bearer xxx` も捕捉）。値部分は **構造境界文字を明示的に除外** + 長さ上限 256 字。**「構造境界文字」の定義**: `\s`（空白文字全般・改行含む）/ `,` / `;` / `)` / `]` / `}` / `"` / `'` / `<` / `>` の文字クラス。`\S+` 過貪欲による JSON 風出力での後続フィールド食い込み・改行混在ケースの誤マスクを回避 |
...
| `C:\\Users\\[^\\]+` / `/Users/[^/]+` / `/home/[^/]+` / `/var/lib/[^/]+` / `/data/users/[^/]+` / `/opt/[^/]+/users/[^/]+` | `<user-home>` | ローカルパス内のユーザ名（標準ユーザディレクトリ + エンタープライズ環境の非標準パスにも対応）。`/opt/[^/]+/users/[^/]+` は `/opt/{vendor}/users/{user}` 形式（vendor 階層必須）専用で、`/opt/users/{user}` 形式は対象外（必要な場合は別途追加）。前方境界を明示しない（`/foo/data/users/...` 等の任意親パス配下も対象とする意図） |
...
コミットハッシュ短縮形 (7〜8 字) ・SemVer (`v1.0.0` 等) ・プラグイン名（XR-1 で 64 字以下に制限済み）
...
| 短い JWT 風文字列「eyJ + `<5字>` + `.eyJ` + `<5字>` + `.<3字>`」（各セグメント 16 字未満） | マスクしない（各セグメント最小長を 16 字に設定して誤検知抑制） |
...
- 40 字閾値: コミットハッシュ短縮形（7〜8 字）・バージョン識別子（v1.x.0 等の数字主体）を
