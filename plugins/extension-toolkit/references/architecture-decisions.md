...
## ADR-015: プラグイン直下ディレクトリの許可リスト厳格運用

| 項目 | 内容 |
|------|------|
| 決定 | プラグイン直下 / `references/` 配下 / スキル直下に許可されるディレクトリ・ファイルを `conventions.md` で **完全列挙** し、列挙されていないものを置くことを禁止する。例外を作る場合は新規 ADR を追加してから許可リストを更新する |
| 理由 | (1) 過去のレビューで `teams/` `templates/` がプラグイン直下に乱立していた問題が発生したため、再発を防ぐ。(2) 構造の予見可能性が利用者・将来の貢献者の認知負荷を下げる。(3) 「何が許可されているか」を SSOT 化することで `validation-rules.md` の機械チェックと連動できる |
| トレードオフ | 新ディレクトリを追加する際の手続コスト（ADR 追加 + 規約更新 + 機械チェック更新）が増える |
| 適用範囲 | プラグイン直下（節 2.1）/ `references/` 配下（節 2.2）/ スキル直下（節 3.1）の 3 階層 |
| 代替案 | ゆるい慣用運用 → 過去の lint 漏れ・乱立を再発、却下 |
