...
| 配置先 | 呼び出し例 | 備考 |
|-------|----------|------|
| プラグイン同梱（同一プラグイン由来） | `bash "$CLAUDE_PLUGIN_ROOT/references/scripts/setup/setup_venv.sh" -WorkDir "<work_dir>" -RequirementsPath "$CLAUDE_PLUGIN_ROOT/references/scripts/setup/requirements.txt"`  | `$CLAUDE_PLUGIN_ROOT` は同プラグイン由来のスキル/コマンド/フック実行時に Claude Code が解決（Bash 標準、shell-preference.md 準拠） |
