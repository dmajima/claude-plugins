...
### 3.1 大規模 eval スイートの簡略様式（条件付き許容）

大規模 eval スイート（目安 50 件超）では、プラグインが自身のドキュメント（`references/CLAUDE.md`・`roadmap.md` 等）で簡略化の意図と段階移行の方針を明文化している場合に限り、「期待出力」表を省略した 4 セクション様式（`入力` / `期待動作` / `分岐の根拠` / `関連ケース`）を許容する。この規模・文書化の条件を満たさない場合は本節の 5 セクション標準に従う（無条件の省略は不可）。
