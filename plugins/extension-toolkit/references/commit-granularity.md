...
`extension-reviewer` の機械チェック・専門家レビューで以下を確認する。

| チェック | 検出対象 |
|----------|---------|
| 1 コミット内のファイル変更数 | 大規模（> 20 ファイル）の場合は分割妥当性をレビューで判断 |
| 1 コミット内のスコープ混在 | コミットメッセージのスコープと変更ファイルパスの不一致を機械検出 |
| ADR 追加と実装の同梱 | `architecture-decisions.md` の変更行数とコミット全体行数の比率 |
