...
本ファイルはディレクトリ構造規約。命名規約は [`conventions-naming.md`](conventions-naming.md)、その他の共通規約・禁止事項は [`conventions-general.md`](conventions-general.md) を参照。
...
| ファイル分離原則 | ルール・ガイド・チェックリスト・手順・テンプレートは別ファイルで管理。1 ファイルに混在させない |
| 命名 | kebab-case + 用途名 |
