...
修正案:
1. ${CLAUDE_PLUGIN_ROOT}/scripts/log.sh に変更（プラグイン同梱スクリプトとして配備）
2. ${HOME}/scripts/log.sh に変更（利用者ホーム配下、ただし存在は前提）
...
| 選択 | 動作 |
|-----|------|
| 1 | `${CLAUDE_PLUGIN_ROOT}/scripts/log.sh` に書き換え、`scripts/log.sh` のスケルトン作成も提案 |
| 2 | `${HOME}/scripts/log.sh` に書き換え、利用者環境への配置案内 |
| 3 | 二重確認 + ハードコード継続を記録 |
| 4 | 何もせず終了 |
...
| 項目 | 期待値 |
|-----|-------|
| 生成ファイル | 選択により異なる（選択 1 の場合: `hooks.json` + `scripts/log.sh` スケルトン） |
