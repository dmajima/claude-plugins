...
| 5 | `references/scripts/{業務}/` の実行可能性確認（ADR-025、スキル直下 `scripts/` は禁止）。Python 利用時、依存はプラグイン直下 `${env:CLAUDE_PLUGIN_ROOT}/references/scripts/setup/requirements.txt` に統合する（ADR-024、スキル単位 `requirements.txt` 禁止）。venv 構築は同階層の `setup_venv.ps1` を `environment-setup-toolkit` 経由で起動（PowerShell 統一） |
