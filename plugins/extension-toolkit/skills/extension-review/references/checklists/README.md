...
`extension-review` がレビュー結果をユーザに報告する **直前に必ず通過させる** 網羅チェックリスト集。`extension-toolkit/references/` 配下の全 SSOT 由来のルールを 1 箇所に統合し、対象種別ごとに整理した。

## 1. 適用範囲とタイミング

| 項目 | 内容 |
|-----|------|
| 適用対象 | `extension-review` のすべての起動経路（チーム起動 / フォールバック並列起動 / 単体エージェント起動 / マーケットプレイス起動） |
