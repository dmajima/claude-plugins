...
`extension-review` が並列で実施する機械的チェックの一覧と実行方法。
...
| 9 | シークレット混入 | プラグイン全体 | Critical | `check_secrets`（`marketplace-publish` の `secret-scan.md` と同等） |
...
詳細パターンと検出ロジックは
[`../../marketplace-publish/references/secret-scan.md`](../../marketplace-publish/references/secret-scan.md)
