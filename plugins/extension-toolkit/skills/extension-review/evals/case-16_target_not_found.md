...
`extension-review` が対象種別の判定を試みる際、ディレクトリが実在しないため事前に存在確認に失敗する。
...
`run_checks.py` 冒頭の `target.exists` 確認に失敗すると `[ERROR] target not found` を出力して exit code 2 で終了するため、`extension-review` 側はこれを検知してエージェント起動をスキップする必要がある。
