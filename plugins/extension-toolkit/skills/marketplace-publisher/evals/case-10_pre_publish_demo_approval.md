...
ADR-032 の必須項目で「marketplace-publisher は公開モード選択の前にデモ承認を取得」と明記。SKILL.md 節 6 にも「公開モード選択の前に必須」「公開はマーケットプレイス越しに利用者に届くため、デモなき公開は禁止」と記載。免責ケース（緊急セキュリティ修正のみ）に該当しない通常公開フローでは本ケースの手順を必ず通る。
