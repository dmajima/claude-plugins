...
| ステップ | 内容 |
|---------|------|
| (a) デモシナリオ準備 | `evals/demo.ps1`（B-3 テンプレート）を新規スキル用に置換し配置 |
| (b) 代表的な正常系を実行 | `pwsh -NoProfile -File evals/demo.ps1 -WhatIf:$false` で dry-run シナリオを実機実行 |
| (c) 主要分岐 1 件以上を実行 | demo.ps1 の Step 2 で別分岐を起動 |
...
| 項目 | 期待値 |
|-----|-------|
| 生成ファイル | `evals/demo.ps1`（テンプレートから置換配置） |
