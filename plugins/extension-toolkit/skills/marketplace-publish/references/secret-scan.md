...
公開対象プラグインにシークレット（API キー・トークン・秘密鍵・`.env` 等）が混入していないかを `git add` 前に検査する仕組み。`marketplace-publish` の「2. プラグイン実体検証」フェーズで **必須** の検査として実行する。
...
検出された場合、`marketplace-publish` は **公開フローを中断** し、以下のメッセージをユーザに提示する:
