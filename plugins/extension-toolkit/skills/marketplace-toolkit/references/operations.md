...
実装時は `[System.IO.Path]::GetFullPath` で正規化したうえで上記拒否リストと照合する（プラグイン直下 `${env:CLAUDE_PLUGIN_ROOT}/references/scripts/setup/teardown_venv.ps1` と同様の 3 段ガード設計、ADR-024、PowerShell 統一）。
