...
| 項目 | 重大度 | 確認方法 | 出典 |
|-----|-------|---------|-----|
| CMD-1-1 | High | ファイル名が kebab-case + `.md`（例: `extension.md`） | [conventions.md](../../../references/policies/conventions-structure.md) 節 1 |
...
| 項目 | 重大度 | 確認方法 | 出典 |
|-----|-------|---------|-----|
| CMD-2-1 | Critical | frontmatter（YAML）が valid（パース可能） | [validation-rules.md](../../../references/checklists/validation-rules.md) 節 1 |
| CMD-2-2 | Medium | `description` が 60 文字以内 | [description-guide.md](../../../references/guides/description-guide.md) 節 4 |
...
| 項目 | 重大度 | 確認方法 | 出典 |
|-----|-------|---------|-----|
| CMD-3-1 | High | 引数を受け取るコマンド（本文に `$ARGUMENTS` を含む）に `argument-hint` が **必ず** 記載されている | [description-guide.md](../../../references/guides/description-guide.md) 節 4.1 / ADR-023 |
...
| 項目 | 重大度 | 確認方法 | 出典 |
|-----|-------|---------|-----|
| CMD-4-1 | High | ルーティング先スキルがすべて存在する | [validation-rules.md](../../../references/checklists/validation-rules.md) 節 2.3 |
| CMD-4-2 | Medium | ルーティング表が表形式で本文に記載されている（外部設定ファイルへの切り出し禁止・ADR-007） | [architecture-decisions.md](../../../references/architecture/decisions-001-010.md) ADR-007 |

## CMD-5. インラインスクリプト禁止（ADR-025）

| 項目 | 重大度 | 確認方法 | 出典 |
|-----|-------|---------|-----|
| CMD-5-1 | High | コマンド本文内に 6 行以上のフェンス付きコードブロック（実行用）がない | [scripts-policy.md](../../../references/policies/scripts-policy.md) 節 3.1 |
...
| 項目 | 重大度 | 確認方法 | 出典 |
|-----|-------|---------|-----|
| CMD-6-1 | High | コマンド本文内のパスが `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_SKILL_DIR}` / 相対パス を使用している | [path-portability.md](../../../references/policies/path-portability.md) |
...
| CMD-8-3 | High | 削除・上書き等の破壊的操作はユーザ確認（AskUserQuestion）を経る | [user-interaction.md](../../../references/guides/user-interaction.md) |
