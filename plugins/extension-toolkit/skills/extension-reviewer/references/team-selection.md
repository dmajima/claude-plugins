...
`TeamCreate` 機能が利用できない環境では、各チーム定義から **メンバー一覧 + スポーンプロンプト** を抽出し、`Agent` ツールでメンバーを **個別並列起動** する。詳細手順とチーム機能との差分は [`../../../references/agent-utilization.md`](../../../references/agent-utilization.md) の「6.5 チーム機能が利用できない環境でのフォールバック」を参照。
