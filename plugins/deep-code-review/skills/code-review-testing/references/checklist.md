# code-review-testing 達成チェックリスト

`code-review-testing` 観点別スキルが **中間レポートを返却する前** に通過すべきルール群。
ID 体系・SSOT は `${CLAUDE_PLUGIN_ROOT}/references/skill-rules-matrix.md` を参照。

> **確認タイミング**: 内部 2 エージェント（test-engineer / test-runner）の結果統合後、オーケストレーターへの返却前。
...
```
[ ] (O1)  2 エージェント（test-engineer / test-runner）を 1 メッセージ内で並列起動している
[ ] (O2)  SKILL.md「出力フォーマット」セクションに従って中間レポートを返している
[ ] (O3)  test-runner は対応 Bash 権限（dotnet/npm/jest/vitest/pytest/pwsh 等）がある場合のみ実行・なければ SKIPPED 記録
[ ] (O4)  E2E / 結合 / ブラウザ / 性能テストはスコープ外として明示している（test-runner はユニットのみ）
[ ] (O5)  各指摘・改善提案にスコープ内/外フラグを付与している
[ ] (O6)  プロジェクト規約（CLAUDE.md / .claude/rules/ / テスト規約）を最優先評価基準にし根拠に引用している
...
| O3 | テスト実行コマンドの権限を確認・足りない場合は SKIPPED 記録 |
| O4 | E2E / 結合等のスコープ外指摘は本レポートから除外 |
