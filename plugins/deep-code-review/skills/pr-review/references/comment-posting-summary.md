...
`pr-review` スキル Step 7 のサマリースレッド投稿の詳細実装。統一フォーマット・投稿順序・署名を扱う。
...
レビュー終了時に PR 全体宛の **サマリースレッド** を投稿する（`threadContext.filePath == null` の thread）。**毎回同一レイアウトで** 投稿し、読み手が同じ位置で同じ情報を見つけられるようにする。
...
- Medium 以下は **設計上の改善余地・命名・ドキュメント整備** 等、軽微で次バージョンへの持ち越し可
...
これにより `auto-resolve=false` 指定時や MD 出力モードでもユーザーが「実投稿時に何をすべきか」を把握でき、手作業や次回フルラン時の手順が明確になる。
...
- 旧サマリーを closed にしてから新サマリーを投稿し、active なサマリーが常に 1 件のみになる
