...
description: deep-code-review の観点別スキル。コード変更をセキュリティ観点（脅威モデル・OWASP/STRIDE・依存安全性）でレビュー。「セキュリティをレビューして」「OWASP/STRIDE で確認して」「依存関係の脆弱性を見て」や認証・データ保護・外部公開の変更時に起動する。Use when reviewing security or dependency safety. SKIP when reviewing implementation/tests/architecture/frontend (use the matching code-review-* skill).
allowed-tools:
  - Read
  - Grep
  - Glob
...
`dependency-safety` は対応 Bash 権限が許可された場合のみ脆弱性スキャンを実コマンドで実行。
