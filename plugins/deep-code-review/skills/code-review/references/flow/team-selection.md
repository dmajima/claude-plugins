...
`code-review` オーケストレーターが Agent Teams（`TeamCreate`）を採用する際の **チーム選定パターン** を事前定義する。差分・性質を確認し、この中から最適なパターンを選んで `TeamCreate` で組織する。
...
> 下記「セクションマップ」「パターンマップ」に保持しており本索引で解決できる。
