...
ユーザー指示からレビュー対象（PR / ブランチ / ファイル / 差分）を確定する手順を定義する。
...
リモート参照（`origin/`）を使うことで、未 fetch なローカルブランチとの差分による誤判定を防ぐ。
...
レビュー対象リポジトリの `CLAUDE.md` / `.claude/rules/` 等に「比較ブランチは XXX を使う」と明記されている場合は、その指定を **最優先**。
自動判定をスキップして指定値を使い、その旨をユーザーに通知する。
...
「PR #N をレビュー」「PR URL をレビュー」「Azure DevOps の PR をレビュー」と依頼された場合は、**`pr-review` スキルへ委譲する**（オーケストレーター内では PR 操作を直接行わない）。
...
差分から以下を分類して、エージェント選定（`agents.md`）に渡す。
...
> プロジェクト固有のディレクトリ構成・モジュール分類はスキル内に保持しない。レビュー対象リポジトリの `CLAUDE.md` / `.claude/rules/` / `README.md` 等から各エージェントが必要に応じて読み取る。
...
非対話モードでは標準モードで全件レビューに進むが、所要時間とコストが大きいことをサマリの「未確認事項・制約」セクションに記録する。
