...
> ```
>
> これは「2xx 以外はすべて即 throw」する保守的フェイル形式で、サンプルコードの認知負荷を抑えるための **教材的省略形** です。**プロダクション実装では下記の完全 switch 分岐**（429 指数バックオフ / 401-403 即停止 / 5xx 単発リトライ）**を使うこと**。簡略形は誤動作を生まない（429 でリトライしないだけで、失敗が安全側に倒れる）ため学習用には十分ですが、本番品質には不足します。
...
リトライで重複が発生してしまった場合（ガード前に既に複数スレッドが残っている場合）の検出は、`completion-checklist.md` E セクションで実施する。
...
Windows PowerShell 5.1 でも動作するが、`pwsh` の方が `?.`・`??`・三項演算子等を含めた構文が充実しているため推奨。

```powershell
# 推奨: switch -Regex パターンマッチ
switch -Regex ([string]$HttpCode) {
    '^2\d{2}$' { <# 成功 #> }
    default { Write-Host "HTTP $HttpCode"; throw 'request failed' }
}

# 代替: 簡略形（2xx 以外は throw）
if ([string]$HttpCode -notmatch '^2\d{2}$') { throw "HTTP $HttpCode" }
```
...
REST API を curl で呼ぶ個別スキルは、本ファイルの規定（HTTP コード取得 + case 分岐 / 401-403 即停止 / 429 指数バックオフ / 5xx 単発リトライ / 部分失敗ロールバック方針）に
準拠することを宣言したうえで利用すること。

本ファイルから個別スキルへの参照は意図的に持たない（共通モジュールが
クライアントを知る必要はないため）。利用側スキルは自身の references から
本ファイルへ片方向で参照する。
