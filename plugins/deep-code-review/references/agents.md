...
ユーザーが明示的に「フルレビュー」「全エージェント動員」を要求した場合、上記の動的省略を適用せず標準モード全動員とする。
...
各エージェント定義（`agents/{name}.md`）の **ロール定義 / 評価観点 / 出力フォーマット** 各節を役割・観点・出力様式の正とし、以下の共通変数・共通ブロック（4.1〜4.4）を組み立てて `prompt` パラメータを構成する（各定義末尾の「プロンプトテンプレート」節はスタブ化済み。組み立て規則は本セクションを正とする）。
