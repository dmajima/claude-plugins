...
- evals は全スキル統一で「入力 / 分岐の根拠 / 期待動作 / 関連ケース」の 4 セクション構成を採用する（extension-toolkit `eval-guide.md` の 5 セクション標準に対する本プラグインの明示的な簡略フォーマット。「期待出力」は期待動作の箇条書きに統合して記載する）。新規ケースは既存最大番号 +1 から採番する
