...
description: |
  ...
  What: 多様な形式の文字起こしデータを読み取り、ailead-fetcher と同一の出力形式に正規化する。
  Where: セッション作業領域に transcript.txt と metadata.json を出力する。
  When: ユーザーが文字起こしテキストを直接貼り付けた場合、VTT/SRT ファイルを提供した場合、または Teams チャットからコピーした文字起こしを提供した場合。
  Why: データソースごとに minutes-composer を分岐させず、統一された中間形式を経由して処理するため。ailead-fetcher と同一の出力形式にすることで、下流スキル（minutes-composer / minutes-reviewer）がソースに依存しない。
  How: 入力形式を自動判定し、発話者・タイムスタンプ・テキストを抽出して標準形式に変換する。
  責務外: 音声ファイルからの文字起こし（STT）。ailead 共有リンクからの取得（ailead-fetcher が担当）。議事録の構造化（minutes-composer が担当）。
trigger:
