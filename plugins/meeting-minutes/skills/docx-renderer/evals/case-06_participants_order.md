# Case 06: 出席者欄の並び順・敬称・bot 除外

## 入力

| 項目 | 値 |
|-----|---|
| 起動条件 | case-01 と同一（正常な docx 生成） |
...
1. `generate_docx.py` の `arrange_participants()` が出席者欄の表示行を組み立てる
...
| 顧客が複数社の場合 | participants 配列の出現順を維持して並べる |
| `role: "host"` が 1 名もいない場合（規約違反入力） | フェイルセーフとして全員顧客扱い（様付き）になる。自社に様が付く方向の安全側動作であり意図仕様 |
| 出席者欄が空（全員 bot 等） | 「（参加者情報なし）」を表示 |

## 分岐の根拠

`minutes-composer/references/schema/minutes-schema.md`「participants の role 設定」: 表示順・敬称・bot 除外はレンダラーが決定する。`md-renderer` の case-03 と同一観点（両レンダラーで仕様を揃える）。

## 関連ケース

- `case-01_normal.md`（正常生成の基本フロー）
- `md-renderer/evals/case-03_participants_order.md`（md 側の同一観点ケース）
