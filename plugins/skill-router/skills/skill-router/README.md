...
このファイルは **人間向けのリファレンス** です。Claude Code がスキル動作中に参照することはありません。スキルの動作本体は `SKILL.md` および `evals/` 配下、ロジック実装は `${CLAUDE_PLUGIN_ROOT}/references/scripts/routing/` 配下を参照してください。
...
└── evals/              # 動作分岐検証用ケース集（27 ケース）
    ...
    ├── case-24_diag_prompt_hook_timeout.md              # 診断: プロンプトフックのタイムアウト切り分け
    ├── case-25_venv_base_repo_isolation.md              # 負例: リポジトリ同梱 venv を実行しない境界
    ├── case-26_embedding_optout_teardown.md             # 変形: 埋め込み無効化時の venv 撤去
    └── case-27_index_name_tamper.md                     # 負例: index の名前偽装を推奨に載せない
```

実行ロジックはプラグイン直下の `references/scripts/routing/` に集約されており、本スキル内には Python ソースを置かない（ADR-024 / ADR-025 準拠）。
...
| ルーティング本体 | `../../references/scripts/routing/route.py` |
| インデクサ | `../../references/scripts/routing/build_index.py` |
