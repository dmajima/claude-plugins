...
**v0.4.0**: **完全ローカルで動作する埋め込みベース意味的類似度判定** をオプトインで搭載しました。外部 API には一切接続せず、`fastembed` + 多言語 MiniLM モデル（約 120MB）で同義語・言い換えを捕捉します。デフォルトは無効で既存挙動を維持しつつ、有効化すると現行ヒューリスティックでは拾えない表現バリエーションを大幅に改善できます。詳細は「埋め込み判定」セクションを参照してください。
