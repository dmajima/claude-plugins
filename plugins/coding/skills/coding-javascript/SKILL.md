...
description: JavaScript（tsconfig なし）の実装・設計を Prettier 既定とプロジェクト規約優先で支援する言語スキル。「JS で実装して」「JS を直して」「Express ルートを書いて」等で起動する。Use when implementing or structuring JavaScript code. SKIP when 4+ files or full phases (use orchestrator-coding), design-only (use orchestrator-design), or TS/other languages (use coding-*).
---
