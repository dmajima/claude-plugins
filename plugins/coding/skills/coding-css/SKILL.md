---
name: coding-css
description: CSS/SCSS のスタイル実装・設計を Google HTML/CSS Style Guide + BEM とプロジェクト規約優先で支援する言語スキル。「CSS を書いて」「Flexbox でレイアウトして」「レスポンシブにして」等で起動する。Use when implementing or structuring CSS styles. SKIP if 4+ files/full phases (orchestrator-coding), design-only (orchestrator-design), or markup changes (coding-*).
---
