---
name: coding-csharp
description: C# のコード実装・構造設計を Microsoft C# 規約とプロジェクト規約優先で支援する言語スキル。「C# で実装して」「この C# を直して」「ASP.NET Core の API を書いて」等で起動する。Use when implementing or structuring C# code. SKIP when 4+ files or full phases (use orchestrator-coding), design-only (use orchestrator-design), or other languages (use coding-*).
---
