# テンプレート（推奨構成の明示管理）

`extension-creator` の各スキルが拡張要素を生成する際に使用する **推奨構成テンプレート**。生成物のムラを抑制するため、推奨構成は本ディレクトリで一元管理する。

## このドキュメントについて

このファイルは人間向けのリファレンス。Claude Code がスキル動作中に参照することはあるが、テンプレート本体への参照はスキルの SKILL.md / references で行う。

## テンプレート一覧

| テンプレート | パス | 利用スキル |
|-------------|------|----------|
| スキル一式 | `skill/` | `skill-creator` |
| プラグイン外形 | `plugin/` | `plugin-creator` |
| スラッシュコマンド | `command/command.md` | `command-creator` |
| サブエージェント | `agent/agent.md` | `agent-creator` |
| エージェントチーム | `agent/team.md` | `agent-creator` |
| フック | `hook/hooks.json` | `hook-creator` |
| README | `readme/README.md` | `readme-creator` |

## 利用方針

| 項目 | 値 |
|-----|---|
| 配置場所 | プラグイン直下 `templates/` で全スキル共有 |
| プレースホルダ | `{kebab-case}` 形式の文字列 |
| 置換タイミング | スキル実行時に値を埋める |
| バージョン管理 | このディレクトリのテンプレートが正典 |

## スキル固有のテンプレートが必要な場合

各スキルは本テンプレートをコピーして `skills/{skill-name}/references/template/` に派生版を作成し、そこから読み込む。本ディレクトリのテンプレートを直接編集すると、全スキルに影響するため注意する。

## プレースホルダ命名規則

| プレースホルダ | 意味 |
|--------------|------|
| `{plugin-name}` | プラグイン名（kebab-case） |
| `{skill-name}` | スキル名（kebab-case） |
| `{command-name}` | コマンド名（kebab-case） |
| `{agent-name}` | エージェント名（kebab-case） |
| `{team-name}` | チーム名（kebab-case） |
| `{author-name}` | 作者名 |
| `{marketplace-name}` | マーケットプレイス名 |
| `{...}` 内のその他 | 文脈で意味が明らかな自由記述 |
