...
`extension-creator` の各 `*-creator` および `extension-reviewer` が共通で参照する検証チェックリスト。同じ検証ルールを複数箇所に書かないようにこのファイルに集約する。
...
### 2.5 エージェントチーム（`agent-creator` チームモード出力）
...
| 参照元 | 参照部分 |
|-------|---------|
| `skill-creator` | 1 + 2.1（スキル出力検証） |
| `plugin-creator` | 1 + 2.2（プラグイン外形検証）+ 種別別の該当節 |
| `command-creator` | 1 + 2.3 |
| `agent-creator` | 1 + 2.4（単体）or 2.5（チーム） |
| `hook-creator` | 1 + 2.6 |
| `readme-creator` | 1 + 2.7 |
