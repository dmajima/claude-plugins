...
- 「既存スキルをプラグイン化」（→ `plugin-creator` の移管シナリオ）
...
2. プラグイン内のアイテム配置が完了している（`plugin-creator` の新規/移管シナリオが完了済み + 必要に応じて `skill-creator` / `command-creator` / `agent-creator` / `hook-creator` で本体生成済み）
...
選択肢:
1. 既存プラグインへのマージ（新プラグインの中身を既存に追加、`plugin-creator` の追加シナリオへ）
2. 新規登録を続行（差別化点を明記）
3. キャンセル
