---
name: plugin-creator
description: Claude Code のプラグイン外形（plugin.json・README・ディレクトリ構造）を新規作成し、既存のスキル/コマンド/フック/エージェントをプラグイン構造へ移管（コピー）するスキル。「新しいプラグイン foo を作って」「既存スキル bar をプラグイン化」「foo プラグインに既存スキル追加」などの依頼で起動する。Use when the user wants to scaffold a new plugin shell or migrate existing assets (skills, commands, agents, hooks) into a plugin structure. SKIP when the user wants to create a skill body (use skill-creator), command body (use command-creator), agent body (use agent-creator), or hook body (use hook-creator). Marketplace registration is delegated to marketplace-publisher.
---
...
| 業務 | 担当スキル |
|-----|----------|
| スキル本体（SKILL.md・references・scripts・evals）の生成 | `skill-creator` |
| スラッシュコマンド本体の生成 | `command-creator` |
| サブエージェント・チーム本体の生成 | `agent-creator` |
| フック設定本体の生成 | `hook-creator` |
...
- 「スキル本体を作って」（→ `skill-creator`）
- 「コマンド本体を作って」（→ `command-creator`）
...
| 次のアクション | 接続先 |
|--------------|-------|
| スキル本体未生成（外形のみ作成済み） | `skill-creator` |
| コマンド本体未生成 | `command-creator` |
| エージェント本体未生成 | `agent-creator` |
| フック本体未生成 | `hook-creator` |
| README の追加カスタマイズ | `readme-creator` |
