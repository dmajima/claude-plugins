---
name: agent-creator
description: Claude Code のサブエージェント（agents/{name}.md）およびエージェントチーム（複数の独立した Claude Code インスタンスによる協調作業）を新規作成・編成・改修するスキル。「コードレビュー用エージェントを作って」「セキュリティ評価チームを編成」「実装エンジニア視点のエージェントが欲しい」などの依頼で起動する。Use when the user wants to create, design, or refactor a sub-agent or an agent team. SKIP when the user wants a skill (use skill-creator), plugin shell (use plugin-creator), command (use command-creator), or hook (use hook-creator).
---
...
| 業務 | 担当スキル |
|-----|----------|
| スキル本体の生成 | `skill-creator` |
| プラグイン外形の生成 | `plugin-creator` |
| スラッシュコマンド生成 | `command-creator` |
| フック設定の生成 | `hook-creator` |
| README 単体の生成 | `readme-creator` |
