...
description: Claude Code のスラッシュコマンド（commands/{name}.md）を新規作成・改修するスキル。「新しい /foo コマンドを作って」「foo コマンドを更新」「○○用のコマンドが欲しい」などの依頼で起動する。Use when the user wants to create or modify a slash command file. SKIP when the user wants to create a skill (use skill-creator), plugin shell (use plugin-creator), agent (use agent-creator), or hook (use hook-creator).
---
...
| エージェント・チームの生成 | `agent-creator` |
| フック設定の生成 | `hook-creator` |
| README 生成・更新 | `readme-creator` |
...
- README 更新が必要な場合は `readme-creator` への接続を提案
