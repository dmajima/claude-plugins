...
| スキル | 担当 |
|-------|------|
| `skill-creator` | スキルの新規作成・既存改修。`example-skills@anthropic-agent-skills` / `document-skills@anthropic-agent-skills` と連携 |
| `plugin-creator` | プラグインの新規作成・既存資産のプラグイン構造化 |
| `command-creator` | スラッシュコマンドの作成 |
| `agent-creator` | サブエージェント・エージェントチームの設計・作成 |
| `hook-creator` | フック設定の作成 |
| `readme-creator` | README（人間向けリファレンス）の生成・更新 |
...
| 発話例 | 起動スキル |
|-------|-----------|
| 「新しいスキル `foo` を作って」 | `skill-creator` |
| 「`bar` プラグインを作成」 | `plugin-creator` |
| 「`/baz` コマンドを作って」 | `command-creator` |
| 「コードレビュー用エージェントチームを設計」 | `agent-creator` |
| 「PreToolUse フックで X したい」 | `hook-creator` |
| 「`qux` スキルの README を書いて」 | `readme-creator` |
...
`skill-creator` は以下のスキルを `Skill` ツール経由で利用する場合があります:
...
```text
/plugin install extension-creator@dmajima-claude-plugins
```
