...
| `automation: exploratory` の例外 | `timeout_sec` はセッションのタイムボックス（計画時間）であり、**超過 = blocked を適用しない**（タイムボックス満了 = セッション正常終了として結果判定へ進む。`manual-execution.md` 6 章） |
...
| `automation: manual-assist` のケース | 実行せず**手順書を一括生成し** skipped + reason（**手順書パス**を含む）を記録する（生成はオーケストレータ。生成失敗時は従来どおり理由のみで skipped。`manual-execution.md` 7 章） |
| `automation: exploratory` のケース | 実行せず**チャーターシート様式で手順書を一括生成し** skipped + reason（シートパスを含む）を記録する（同上。`manual-execution.md` 7 章） |
