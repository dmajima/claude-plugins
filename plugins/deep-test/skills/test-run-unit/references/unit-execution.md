...
4. ランナー実体を確認する（`--version` 等の無害なコマンドで起動可否を確認）
...
| 未導入 | ランナー・依存パッケージの導入は行わない（環境構築は `test-setup` の責務）。実行不能なら skipped + reason で返却する |
...
test-cases.yaml のケースは、`data` または `steps` にテスト名・パターンを記載している前提とする（設計時の推奨は `data` への記載）。
...
- 一括実行時の `duration_sec` は、ランナー出力の個別テスト時間を採用できる場合はそれを記録し、取得できない場合は全体時間を案分せず null とする（値を偽装しない）
...
- リトライ・再実行で結果を上書きしない。フレーク（不安定テスト）を検出した場合は、実際の結果と再現率を actual に記録する
