...
`automation: exploratory` の uat スコープのケースについて、対話時はチャーター提示 → タイムボックス案内 → セッション実施（AI は書記 + 操作補助）→ 終了聴取（ノート・発見・PROOF）→ セッションシートの evidence 化を経て、`executed_by: human-assisted` + `defect.extras.session_findings` で記録することを検証する。非対話モードでチャーターシート縮退（skipped + reason）になる分岐は case-12 が扱う。
...
| 前提 | 対話モード（非対話のチャーターシート縮退は case-12）。Playwright MCP はロード済み（AI の操作補助に使用可）。セッション開始が成立する（開始不能分岐は case-14） |
