...
- **resume 時（副分岐 3）**: 中断 run の resume（flow-resume.md 5.1 手順 6）で既存 `environment.yaml` を再利用する場合も、`applicability: applicable` 判定の前に本検証を同形で適用する（中断中に破損したマニフェストをそのまま稼働確認に用いない）
