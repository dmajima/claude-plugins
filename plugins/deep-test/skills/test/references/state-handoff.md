...
- オーケストレータは成果物ファイル（`environment.yaml`）のパス存在を確認し、加えて YAML として parse 可能であることを venv Python で機械確認する（失敗は再委譲 1 回 → 環境なし縮退・venv 不在は目視縮退。手順は flow.md 6 章 Phase 1.7 節「受領後の parse 検証」）。Markdown 要約をユーザー報告に用いる（返り値の JSON パースは行わない）。`environment.yaml` を parse 検証するのは、下流の機械処理（`start-run --environment` の環境文字列材料化等）が parse 可能を前提に消費するため（analysis.yaml〔2.2〕/ fixtures.yaml〔2.3〕は LLM が Read で読解して材料化するため機械 parse 検証を課さない = 意図された非対称）。`environment.yaml` は下流が単方向に消費する（test-environment へ戻さない）
