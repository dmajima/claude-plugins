...
- 要素操作は「browser_snapshot で要素 ref を取得 → 操作ツール（browser_click / browser_type 等）に ref を渡す」を基本とし、各ステップ後にスクリーンショットを取得し直後に移送する（5 章）
...
モジュール間のデータ整合（登録 → 参照・更新 → 反映）は、以下の手順で確認する。
...
- 疎通不可が判明した場合は 3 章のスタブ判断へ進む（即座に skipped とせず、まずポリシー判断する）
