...
description: SUT の docker 資産から非破壊でテスト用派生環境（compose.test.yml・.env.test）を生成し、up/down/status のライフサイクルと environment.yaml を管理するフェーズスキル。analysis.yaml を消費し endpoints/exec_forms を下流へ提供。責務外=テスト実行(test-run-*)・ツールチェーン検証(test-setup)・ケース設計(test-design)。test 委譲時や「テスト用コンテナ環境を作って」「テスト用コンテナ環境を起動して」「テスト用コンテナ環境を片付けて」と依頼された時に使用。
allowed-tools:
  - Read
  - Grep
  - Glob
...
SUT の docker 資産（compose / Dockerfile / `.env` 系）から**非破壊で**テスト用派生環境（`environment/compose.test.yml` / `environment/.env.test`）を生成し、そのライフサイクル（provision / up / down / status）と機械可読マニフェスト `environment.yaml` を管理する Phase 1.7 のフェーズスキル。
`test-analyze` の `analysis.yaml` を材料に単方向消費し、`endpoints[]`（browser 系実行スキルへのテスト用 base URL）と `exec_forms[]`（コンテナ内ランナー実行形）を下流へ提供する。**テストの実行そのものはしない**（SUT が動く環境を用意して止まるのが本スキルの境界）。

## 責務

| # | 責務 | 概要 |
|---|------|------|
| 1 | Docker 資産検出・要否判定 | `project=` 起点に compose（`compose.y*ml` / `docker-compose.y*ml` / `compose.override.y*ml`）・`Dockerfile*`・`docker/`・`.env` 系の**有無のみ**を Glob で検出し、`docker compose version` で v2 疎通を確認する。資産なし / `levels=` が unit のみ / docker 不可なら no-op / 縮退 |
| 2 | analysis.yaml 消費 | `architecture.build_run`・`dependency_summary.external_dependencies`・`meta.target_type`・`entry_points` を派生方針（分離対象・モック profiles・本番誤爆疑義）の材料に単方向消費する。非存在時は軽量補完（`analysis_consumed: false`） |
| 3 | 派生成果物の生成（非破壊） | `environment/compose.test.yml`（ports は `!override` で全置換 + 127.0.0.1 バインド・bind mount の `ro` 化 / named volume 再定義・モック系サービスは profiles 配下）と `environment/.env.test`（ダミー値 / credentials-manager 参照形のみ）を Write する。SUT の既存 docker 資産は一切変更しない |
...
| 6 | 実行形の提供 | `endpoints[]` と `exec_forms[]` を environment.yaml に記録して下流へ引き継ぐ（提供形の記録のみ。実行はしない） |
| 7 | teardown（action=down） | サービス別 logs 保存 → `down -v --remove-orphans`（up と同一 `-f` 群 + `-p` に固定）→ `ps` で残存確認 → status 更新 |
| 8 | environment.yaml 出力 | meta / derived_from / derived_artifacts / project / services / endpoints / exec_forms / lifecycle / status を機械可読で出力・更新する。`action=status` は `ps` + health 再確認で status のみ更新する |
| 9 | 自己チェック | `env-architect` エージェントで派生設計の分離妥当性・read-only 境界・秘匿値の非出力・本番誤爆疑義・teardown 完全性・スキーマ準拠を単独レビューし、重大指摘を反映してから返却する |
...
- オーケストレータ `test` から Skill ツール経由で委譲された場合（フルフローの Phase 1.7 provision・全ゲート通過後の up〔Phase 5 手順 0〕・Phase 6 判定後の down）
- 「テスト用のコンテナ環境を作って」「テスト用コンテナ環境を起動して」「テスト用コンテナ環境を片付けて」「compose からテスト環境を派生して」と依頼された場合

起動しないケース:

- テストの実行・実走を求められた場合（`test-run-*` の責務）
- Playwright MCP 登録・ランナー検出等のツールチェーン検証を求められた場合（`test-setup` の責務）
- 対象アプリの一次解析（analysis.yaml 生成）を求められた場合（`test-analyze` の責務）
- フィクスチャ・seed コードの生成を求められた場合（`test-fixture` の責務）

## 前提

- `${CLAUDE_PLUGIN_ROOT}/references/` の共通規範（yaml-schema-environment.md / yaml-schema-analysis.md / data-locations.md / execution-policy.md / agents.md）が存在すること
- `env-architect` エージェント定義がプラグインルート `agents/` に存在すること
- `analysis.yaml`（`test-analyze` 生成）が存在すれば材料に消費する。無ければ Read/Glob/Grep で軽量補完する
...
| 判定条件 | モード | 動作 |
|---------|-------|------|
| 引数に `--non-interactive` を含む（委譲時はオーケストレータが付与） | 非対話 | 曖昧確認をせず進行する。up は**許可**（down までのワンサイクル完結を条件）・health 未達は down して blocked 材料（`execution-policy.md` 9 章の既定値表）。target-slug は `data-locations.md` 4.2 章の非対話規則。本番誤爆疑義はモック / ダミー値へ差し替え、差替不能なら up へ進まない |
| 上記以外 | 対話 | 不足情報（target-slug・project）・本番誤爆疑義の扱い・health 未達 / up 失敗時の維持・down・ユーザー起動 URL 提示をユーザーに確認する |
...
### 1. 入力解決・action 確定
引数を解釈し、`action=` / `project=` / target-slug を確定する（委譲時は受領値、単独時は解決フロー）。up / down / status では既存 `environment.yaml` を Read で確認する。

### 2. 資産検出・v2 疎通（provision）
`project=` 起点に docker 資産の**有無のみ**を Glob で検出し、`docker compose version` で v2 疎通を確認する（内容・値は読まない）。
...
資産なし / `levels=` が unit のみ / docker 不可なら、派生せず no-op マニフェスト（`applicability` + `reason`）を出力して正常終了する（縮退表は procedures 9 章）。

### 5. 派生生成（provision）
`compose-derivation.md` に従い `environment/compose.test.yml`（`ports: !override` + 127.0.0.1）と `environment/.env.test` を Write する（SUT 側へは書かない）。

### 6. config 検証（provision）
コマンド規約形 + `config --quiet` で静的検証する。失敗時は派生成果物を残し `config_validated: false` + 理由で返却する（up へ進まない）。

### 7. environment.yaml 出力（provision）
`yaml-schema-environment.md` に完全準拠して `{base}/{target-slug}/environment.yaml` を Write する（endpoints の health は `unknown`）。

### 8. 自己チェック（provision）
`env-architect` を単独起動し、重大指摘を反映してから返却する（評価のみ・修正は本スキル）。

### 9. 起動・health 確認（action=up）
`docker version` 疎通 → `up --wait --wait-timeout {N}` → healthcheck 未定義サービスのみ curl 補助 → endpoints / exec_forms を確定し status を更新する。

### 10. teardown（action=down）
サービス別 logs を保存（run_id あり: `evidence/{run_id}/environment/`・なし: `environment/logs/{timestamp}/`）→ `down -v --remove-orphans` → `ps` 残存確認 → status 更新。

### 11. 状態再確認（action=status）
`ps` + health 再確認で `status` のみ更新する（resume / retest の再利用判定に使う）。
...
- **read-only 境界**: SUT の docker 資産（compose・Dockerfile・`.env` 系）・SUT ソースへ**一切書き込まない**。Edit を持たず、Write の書き込み先を `{base}/{target-slug}/environment.yaml`・`{base}/{target-slug}/environment/` 配下・ログ保存先（`evidence/{run_id}/environment/`）に**限定**する運用（本 SKILL の制約+検証チェックリスト+env-architect 自己チェック）で read-only 境界を担保する。`test-results.yaml` / `test-cases.yaml` / `analysis.yaml` / `fixtures.yaml` へも書き込まない
