---
name: test-analyze
description: テスト対象ソースを read-only で静的に理解し、下流が消費する解析材料（analysis.yaml / target-analysis.md）を生成する Phase 1.5 のフェーズスキル。決定は行わず提案（hint）に留める。責務外=ケース設計・レベル/技法/優先度決定（test-design が担当）、テスト実行・カバレッジ実測（test-run-* が担当）。test 委譲時や「テスト対象を解析して」「解析材料を作って」「テスト対象のリスクを洗い出して」と依頼された時に使用。
allowed-tools:
  - Read
  - Grep
  - Glob
...
テスト対象ソースを read-only で静的に理解し、下流スキルが消費するテスト対象理解の**材料（evidence）**を生成する Phase 1.5 のフェーズスキル。
生成物は機械可読の `analysis.yaml` と人間可読の `target-analysis.md` であり、test-design がこれを材料に**レベル / 技法 / 優先度 / ケースを決定する**（決定の直前で止まるのが本スキルの境界）。

## 責務

| # | 責務 | 概要 |
|---|------|------|
| 1 | アーキ・技術スタック把握 | 言語 / フレームワーク / レイヤー / ビルド・実行基盤を構造化する |
| 2 | 依存グラフ・エントリポイント抽出 | import / 呼出関係と HTTP ルート / API / CLI / メッセージ購読 / スケジュールジョブ / UI 画面 / 公開関数を列挙する |
| 3 | 複雑度・変更頻度（ホットスポット） | 循環的複雑度（計測ツール有時のみ）× git churn で高リスク Top N を特定する。ツール無しは `measured: false` |
| 4 | 既存テスト資産分析 | 既存テストの枠組み / 対象 / 粒度を要約し、疑わしい空白を推定する |
| 5 | テスタビリティ評価 | DI 欠如 / グローバル状態 / 隠れ I/O / 非決定性 / 時刻結合と seam 候補を検出する |
| 6 | 変更影響分析（`diff=` 時） | 変更ファイル → 依存逆引き → 影響モジュール / EP と回帰スコープ候補を算出する（**提案のみ**） |
| 7 | 仕様乖離検出（`spec=` 時） | 主要ルート / ルールを仕様と粗く突合し乖離を記録する |
| 8 | 対象種別判定 | web-app / api / batch / library / data-pipeline / cli / mixed / unknown を判定する |
| 9 | リスクレジスタ算出 | likelihood（複雑度 / churn / 外部依存）× impact（露出 / 業務重要度）で risk_level を算出する（ISTQB product risk） |
| 10 | 攻撃面・軽量脅威モデリング | 公開 EP・信頼境界から STRIDE 6 分類の静的所見を付与する |
| 11 | カバレッジ観点の提示 | 必要網羅基準（statement / branch 等）と計測コマンド案を提示する（**実測はしない**） |
| 12 | 品質特性マッピング | ISO/IEC 25010:2023 の 9 特性を対象・EP から判定して材料化する |
| 13 | 自己チェック | `source-analyst` エージェント（`${CLAUDE_PLUGIN_ROOT}/references/agents.md`）で材料の網羅性・根拠妥当性を単独レビューし、重大指摘を反映する |
...
- オーケストレータ `test` から Skill ツール経由で委譲された場合（フルフローの Phase 1.5 analyze フェーズ、変更影響分析 / 仕様乖離検出の依頼）
- 「テスト対象を解析して」「解析材料を作って」「テスト対象のリスクを洗い出して」と依頼された場合

起動しないケース:

- テストケース・テスト計画の設計を求められた場合（`test-design` の責務）
- 材料を使ったレベル / 技法 / 優先度の**決定**を求められた場合（`test-design` の責務。本スキルは決定の直前で止まる）
- テストの実行・カバレッジ実測を求められた場合（`test-run-*` の責務）
- 設計済み成果物のレビュー・承認を求められた場合（`test-review` の責務）

## 前提

- `${CLAUDE_PLUGIN_ROOT}/references/` の共通規範（yaml-schema-analysis.md / data-locations.md / agents.md / execution-policy.md）が存在すること
- `source-analyst` エージェント定義がプラグインルート `agents/` に存在すること
- Bash は read-only の git 読み取りコマンド（`git log` / `git diff` / `git shortlog`）と、**存在する場合のみ**複雑度計測ツール（radon / lizard 等）に限定する。SUT のプロダクションコードへは書き込まない（書き込み境界厳守）
...
> 上流連携: `test-setup`（Phase 1）が検出したランナー・複雑度 / カバレッジツール情報を（利用可能なら）受け取り、ツール有無判定に用いる。無ければ自力検出する。

## 実行モード判定

| 判定条件 | モード | 動作 |
|---------|-------|------|
| 引数に `--non-interactive` を含む（委譲時はオーケストレータが付与） | 非対話 | 曖昧確認をせず解析を進行する。target-slug は `data-locations.md` 4.2 章の非対話規則（唯一の既存 slug 採用・複数はエラー中断）に従う |
| 上記以外 | 対話 | 対象・target-slug の不足情報を AskUserQuestion で確認する |

## 実行フロー

詳細手順は `${CLAUDE_SKILL_DIR}/references/procedures.md`、エージェント運用は `${CLAUDE_SKILL_DIR}/references/agents.md` に従う。本スキルは deep-test ライフサイクルの **Phase 1.5**（`test-setup` の後・`test-design` の前）に位置する。

### 1. 引数解釈・target-slug 確定
引数を解釈し、target-slug を確定する（委譲時は受領値、単独時は解決フロー）。

### 2. source_availability 判定
ソース取得可否を `full` / `partial` / `none` で判定する（縮退動作の分岐キー。yaml-schema-analysis.md 16 章）。

### 3. 材料の生成
`source_availability` に応じて責務 1〜12 を解析する。`full` は全解析、`partial` は取得可能範囲を解析し欠落を `open_questions` へ、`none` はコードベース解析（複雑度・churn・依存グラフ・seam）をスキップし spec / 公開仕様から静的導出する（`confidence: low`）。数値は計測ツールが無ければ `measured: false` + `null`（捏造禁止）。

### 4. 出力生成
`{target-slug}/analysis.yaml`（機械可読・yaml-schema-analysis.md 準拠）と `{target-slug}/target-analysis.md`（人間可読）を Write で生成する。配置は `data-locations.md` に準拠する。

### 5. 自己チェック
`source-analyst` エージェントを単独起動し、材料の網羅性・根拠妥当性・誠実性をレビューさせる。重大指摘を材料へ反映する（反映は本スキルが行い、エージェントには修正させない）。

### 6. 返却
検証チェックリストを通過させ、解析結果サマリを返却する。
