...
3. 解析対象のコード実態も無い（実装前の）プロジェクトの場合は、`harness-define`（spec-first。コミット 0 件から実行できる）への切替を案内する

## 期待出力

- コミットが必要な理由（同期基準に HEAD の SHA を使う）と再実行手順の案内
- コード実態も無い場合: `/project-harness:define` への切替案内
...
procedures.md Phase 1 の検査表「コミット有無」行。case-05（`.git` 自体がない）とは異なり git リポジトリではあるため、`git init` 提案ではなく初回コミット案内となる。コードが存在する場合は本スキルの守備範囲（初回コミット後に解析可能）だが、コードも無い場合は spec-first の `harness-define` が適切なため切替を案内する。

## 関連ケース

- [case-05](case-05_not_git_repo.md): `.git` 自体が存在しないプロジェクト
- `harness-define` evals case-06: コミット 0 件からの spec-first 構築（初回コミットの承認ゲート）
