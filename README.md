...
|----------|------|----------|----------|
| `credentials-manager` | Manage authentication credentials (API keys, tokens, passwords) across Claude Code sessions. Stores credentials in a scope-aware JSON file (project-local when inside a git repo, user-global otherwise) and auto-applies them by URL/domain match. | 0.1.0 | `/plugin install credentials-manager@dmajima-claude-plugins` |
| `extension-toolkit` | Claude Code 拡張要素（プラグイン/スキル/コマンド/エージェント/フック）の作成からマーケットプレイス構築・公開まで支援 | 1.0.0 | `/plugin install extension-toolkit@dmajima-claude-plugins` |
